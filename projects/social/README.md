...
100 users with an average of 10 friends each would mean 1000 total friendships. We would need to call `add_friendship()` 500 times, because each call to that method actually creates _two_ friendships (A becomes friends with B _and_ B becomes friends with A), not just one.

2. If you create 1000 users with an average of 5 random friends each, what percentage of other users will be in a particular user's extended social network? What is the average degree of separation between a user and those in his/her extended network?

About 99% of users will be in any particular user's extended network, with an average degree of separation of about 5.5.
